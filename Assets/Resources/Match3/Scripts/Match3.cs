...
namespace CodeMonkey
{
    ...
    public class Match3 : MonoBehaviour
    {
        ...
        public delegate void OnMatch(Element MatchingGemElement, int Score, int Damage);
        public OnMatch OnGemMatch;
        ...
        #region Non-CodeMonkey Addons
        private float timeRemaining;
        public Grid<GemGridPosition> Grid => grid;

        public List<Target> Enemies = new List<Target>(); //Serialized for Testing purposes

        public Target CurrentSelectedEnemy;
        [SerializeField] private Transform EnemySpot;

        public AudioSource OnMatchAudio; 

        private float ConstantScoreMultiplier = 1, ConstantDamageMultiplier = 1;

        [SerializeField] DamageText EnemyDamageText, PlayerDamageText;
        #endregion
        ...
        public IEnumerator Timer()
        {
            ...
            if (timeRemaining > 0)
            {
                ...
                Match3UI.TimerUpdate(timeRemaining); 
            }
        }

        public void SetMultiplier(float ScoreMultiplier, float DamageMultiplier)
        {
            this.ConstantScoreMultiplier = ScoreMultiplier; this.ConstantDamageMultiplier = DamageMultiplier; 
        }

        private void Start()
        {
            levelSO = GameManager.singleton.GameLoader.GetSceneToLoad(); 
            ...
            OnWin += Match3_OnWin;
            OnLose += Match3_OnLose;

            OnWin += LevelSOScoreReset;
            OnLose += LevelSOScoreReset;

            OnGemMatch += (MatchElement, Score, ctx_3) => score = ScorePoints(GetScore(), Score, ConstantScoreMultiplier, MatchElement);
            OnGemMatch += (ME, Score, Damage) =>
            {
                if (levelSO.goalType == LevelSO.GoalType.DefeatEnemies && CurrentSelectedEnemy != null)
                    EnemyDamageText.SpawnNewText(CurrentSelectedEnemy.DamageCalc(Damage, ME, ConstantDamageMultiplier));
            };

            OnGemMatch += (ME, Score, Damage) => GameManager.singleton.audioManager.PlayClip(OnMatchAudio, 6);
            OnGemMatch += (ME, Score, Damage) => PlayerObject.singleton.Player.Slot_1?.GetPower()?.OnMatch(ME); 
            OnGemMatch += (ME, Score, Damage) => PlayerObject.singleton.Player.Slot_2?.GetPower()?.OnMatch(ME); 
            
            OnGemMatch += (ME, Score, Damage) => PlayerObject.singleton.Player.Slot_1?.GetPower()?.OnMatch(ME);
            OnGemMatch += (ME, Score, Damage) => PlayerObject.singleton.Player.Slot_2?.GetPower()?.OnMatch(ME);
            

            SetMultiplier(PlayerObject.singleton.Player.ScoreMultiplier, PlayerObject.singleton.Player.DamageMultiplier);

        }


        private void LevelSOScoreReset(object sender, EventArgs e)
        {
            if (levelSO.goalType == LevelSO.GoalType.Score || levelSO.goalType == LevelSO.GoalType.TimeTrial)
                levelSO.ResetScore(); 
        }
        ...
        {
            PlayerObject.singleton.Player.TotalNodesCompleted++;
            PlayerObject.singleton.Player.Coins++;

            if (levelSO.name.Contains("Boss"))
            {
        ...
        public void SetLevelSO(LevelSO levelSO)
        {
            ...
            this.levelSO.Scale(PlayerObject.singleton.Player.RunStreak, PlayerObject.singleton.Player.SavedData.CurrentNodeValue); 
            ...
            if (levelSO.goalType == LevelSO.GoalType.DefeatEnemies)
            {
                var EnemySOs = Resources.LoadAll<EnemySO>("ScriptableObjects/Enemies");
                var EnemyPrefab = Resources.Load<GameObject>("Prefab/EnemyPrefab");

                for (int i = 0; i < levelSO.NumberOfEnemeis; i++)
                {
                    var newEnemy = Instantiate(EnemyPrefab, EnemySpot); //Add a place where the enemy is meant to be created
                    var enemyObject = newEnemy.GetComponent<Target>();

                    enemyObject.Enemy = EnemySOs[UnityEngine.Random.Range(0, EnemySOs.Length)];
                    enemyObject.Create();

                    var Location = GameObject.Find("EnemySpot");
                    newEnemy.transform.localPosition = Vector3.forward * 10;
                    newEnemy.gameObject.SetActive(false); 
                }

                if (levelSO.name.Contains("Boss"))
                {
                    var bosses = Resources.LoadAll<EnemySO>("ScriptableObjects/Boss");
                    var routeBoss = Instantiate(EnemyPrefab, EnemySpot);
                    var bossObject = routeBoss.GetComponent<Target>(); 

                    switch (levelSO.name)
                    {
                        case "Boss_A":
                            bossObject.Enemy = bosses[0];         
                            break;
                        case "Boss_B":
                            bossObject.Enemy = bosses[1]; 
                            break;
                        case "Boss_C":
                            bossObject.Enemy = bosses[2]; 
                            break;
                        case "Boss_D":
                            bossObject.Enemy = bosses[3];
                            break;
                    }

                    bossObject.Create();
                    var Location = GameObject.Find("EnemySpot");
                    routeBoss.transform.localPosition = Vector3.forward * 10;
                    routeBoss.gameObject.SetActive(false);

                }
                CurrentSelectedEnemy.SetSliderHealth();
                CurrentSelectedEnemy.gameObject.SetActive(true);
            }


            OnLevelSet?.Invoke(this, new OnLevelSetEventArgs { levelSO = levelSO, grid = grid });

            if (levelSO.goalType == LevelSO.GoalType.TimeTrial)
            {
                timeRemaining = levelSO.MaxTime; 
                StartCoroutine(Timer());
                moveCount = 0; 
            }
        ...
        public void SetMusic()
        {
            ...
            GameManager.singleton.SetSong(musicIndex);
        }
        ...
        public void RemoveEnemy(Target Enemy)
        {
        ...
        {
            RemoveEnemy(Enemies[index]); 
        }

        public Target GetEnemyAtIndex(int index)
        {
            return Enemies[index]; 
        }

        public Target GetEnemy(Target Enemy)
        {
            if(Enemies.Contains(Enemy)) return 
                    
                    GetEnemyAtIndex(Enemies.IndexOf(Enemy));

            return null; 
        }

        public int GetEnemyIndex(Target target)
        {
            return Enemies.IndexOf(target); 
        }

        public void AddNewEnemy(Target Enemy)
        {
            Enemies.Add(Enemy); 
        }
        ...
        {
            CurrentSelectedEnemy?.gameObject.SetActive(false);
            CurrentSelectedEnemy = target;
            CurrentSelectedEnemy.gameObject.SetActive(true); 
        }
        #endregion
        ...
        public bool GetTimeTrailEnded() 
        {
            return score >= levelSO.targetScore && !HasTimeRemaining(); 
        }

        public float ReturnTimeRemaining() => timeRemaining; 
        #endregion


        private int ScorePoints(int OldScore, int MatchValue, float ScoreMultiplier, Element element)
        {
            if(PlayerObject.singleton.Player.HasBonusElement(element) && ScoreMultiplier > 0)
            {
                return ScorePoints(OldScore, Mathf.RoundToInt(MatchValue * ScoreMultiplier));
            } 
            else
            {
                return ScorePoints(OldScore, MatchValue); 
            }
        }
        private int ScorePoints(int OldScore, int MatchValue)
        {
            return OldScore += MatchValue; 
        }
        ...
        public bool TryFindMatchesAndDestroyThem()
        {
            ...
            foreach (List<GemGridPosition> linkedGemGridPositionList in allLinkedGemGridPositionList)
            {
                var Gem = GetGemSO(linkedGemGridPositionList[0]); 

                Element MatchElement = Gem != null ?
                    Gem.Element : GetGemSO(linkedGemGridPositionList[linkedGemGridPositionList.Count -1]).Element;
                ...
                {
                    // More than 4 linked
                    OnGemMatch.Invoke(MatchElement, 200, 25);                    
        ...
        public void TryDestroyGemGridPosition(GemGridPosition gemGridPosition)
        {
            ...
            {
                OnGemMatch.Invoke(GetGemSO(gemGridPosition).Element, 100, 10);
        ...
        private GemSO GetGemSO(GemGridPosition position)
        {
            return GetGemSO(position.GetX(), position.GetY());
        }
        ...
        {
            if (!HasTimeRemaining() && score < levelSO.targetScore && !HasMoveAvailable() || PlayerObject.singleton.Player.CurrentHealth <= 0 || 
                levelSO.goalType == LevelSO.GoalType.Glass && !HasMoveAvailable() && GetGlassAmount() > 0)
            {
