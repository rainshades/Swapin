...
namespace CodeMonkey.Match3Game
{
    /*
    ...
    public class Match3Visual : MonoBehaviour
    {
        ...
        public void Setup(Match3 match3, Grid<Match3.GemGridPosition> grid)
        {
            ...
            cameraTransform.position = new Vector3(grid.GetWidth() * .5f, grid.GetHeight() * 1.25f + cameraYOffset, cameraTransform.position.z);
            ...
            isSetup = true;
            PlayerObject.singleton.Player.Slot_1?.GetPower()?.OnMatchStart(match3); 
            PlayerObject.singleton.Player.Slot_2?.GetPower()?.OnMatchStart(match3);
        ...
        private void Update()
        {
            ...
            switch (state)
            {
                ...
                case State.WaitingForUser:
                    ...
                    if ((match3.GetLevelSO().goalType == LevelSO.GoalType.TimeTrial && !match3.HasTimeRemaining())
                        || PlayerObject.singleton.Player.CurrentHealth <= 0)
        ...
        public void setTryFindMatches() => SetBusyState(1.0f, () => SetState(State.TryFindMatches)); 
